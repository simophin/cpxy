...
use crate::utils::{copy_duplex, write_json_lengthed_async};
use anyhow::anyhow;
...
async fn serve_tcp_proxy_common(
    ...
    let upstream = match s {
        Some(Ok((bound_address, socket))) => {
            write_json_lengthed_async(&mut src, ProxyResult::Granted { bound_address }).await?;
            socket
        }
        None => {
            write_json_lengthed_async(&mut src, ProxyResult::ErrTimeout).await?;
